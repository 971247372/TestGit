...
// import EtlLinkModel from './EtlLinkModel';
// import { PointModel } from '../Common';

const { PointModel } = SRD;

export default class EtlLinkWidget extends React.Component {
  static defaultProps = {
    color: 'black',
    width: 3,
    link: null,
  ...
  drawLine() {
    ...
    paths.push(
      this.generateLink({
        ...
        d: ` M${pointLeft.x} ${pointLeft.y} C${pointLeft.x + margin} ${pointLeft.y} ${pointRight.x -
          margin} ${pointRight.y} ${pointRight.x} ${pointRight.y}` // eslint-disable-line
      })
    );
  ...
  drawAdvancedLine() {
    ...
    if (smooth) {
      ds.push(
        ` M${points[0].x} ${points[0].y} C ${points[0].x + 50} ${points[0].y} ${points[1].x} ${
          points[1].y
        } ${points[1].x} ${points[1].y}` // eslint-disable-line
      );
      ...
      ds.push(
        ` M${points[i].x} ${points[i].y} C ${points[i].x} ${points[i].y} ${points[i + 1].x - 50} ${
          points[i + 1].y
        } ${points[i + 1].x} ${points[i + 1].y}` // eslint-disable-line
      );
    } else {
